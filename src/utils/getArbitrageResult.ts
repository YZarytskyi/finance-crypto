...
export const getArbitrageTableResult = ({
  ...
  const currencies = result.map((x) => {
    const filterPrice = allCurrencies.find((y: Currencies) => y.symbol === x.symbol)
    return filterPrice
  })
    ...
    const map2 = pair2.map((currencyObj) => {
      ...
      const map3 = pair3.map((currencyObj) => {
        ...
        if (
          ...
          result = (100 / pair1AskPrice / pair2AskPrice) * pair3BidPrice - 100
        }
        if (
          pair1Symbol === `${filterPair1}USDT` &&
          pair2Symbol === `${filterPair2}${filterPair1}` &&
        ...
        if (
          ...
          result = (100 / pair1AskPrice) * pair2BidPrice * pair3BidPrice - 100
        }
        if (
          pair1Symbol === `${filterPair1}USDT` &&
          pair2Symbol === `${filterPair1}${filterPair2}` &&
          ...
          result = ((100 / pair1AskPrice) * pair2BidPrice) / pair3AskPrice - 100
        }
        if (
          ...
          result = ((100 * pair1BidPrice) / pair2AskPrice) * pair3BidPrice - 100
        }
        if (
          pair1Symbol === `USDT${filterPair1}` &&
          pair2Symbol === `${filterPair2}${filterPair1}` &&
          ...
          result = (100 * pair1BidPrice) / pair2AskPrice / pair3AskPrice - 100
        }
        if (
          pair1Symbol === `USDT${filterPair1}` &&
          pair2Symbol === `${filterPair1}${filterPair2}` &&
        ...
        if (
          ...
          result = (100 * pair1BidPrice * pair2BidPrice) / pair3AskPrice - 100
        }
        return {
          pair1: pair1Symbol,
