...
import CoinsSelect from './CoinSelect/CoinsSelect'
          ...
          <ul className={style.coinsList}>
            {marketsHome.map((coin) => (
              <li className={style.item} key={coin.id}>
                <NavLink to={`/crypto/coins/${coin.id}`} className={style.cryptoLeftSide}>
                ...
                <div className={style.cryptoRightSide}>
                  <div className={style.cryptoRightTop}>
                    <CoinsSelect
                      ...
                      coinId={coin.id}
                    />
                    <p className={setClassNamePlusOrMinus(coin.price_change_percentage_24h)}>
                      {coin.price_change_percentage_24h.toFixed(1)}%
                    </p>
                    <p className={style.price}>{price(coin)}</p>
                  </div>
                  <NavLink to={`/crypto/coins/${coin.id}`}>
                    <div className={style.chart}>
                      <ChartHome sparkline={coin.sparkline_in_7d.price} coinId={coin.id} />
                    </div>
                  </NavLink>
                </div>
              </li>
