...
const SelectedCoin = () => {
  ...
  const coinDescWithoutTags = selectedCoin?.description.replace(/<a\s+href="(.*?)">.*?<\/a>,?/g, '')
      ...
      <div className={style.descriptionChart}>
        <div className={style.description}>
          ...
          <div className={style.imgName}>
            <img src={selectedCoin.image} width={55} height={55} alt={selectedCoin.name} />
            <p>{`${selectedCoin.name} (${selectedCoin.symbol.toUpperCase()})`}</p>
          </div>
          <div className={style.pricePercentage}>
            <div>{selectedCoin.current_price} $</div>
            <div className={setClassName(selectedCoin?.price_change_percentage_24h)}>
          ...
          <div className={style.marketCapInfo}>
            ...
            {selectedCoin.fully_diluted_valuation && (
              <div>
                ...
                <div>{setNumberFormat(selectedCoin.fully_diluted_valuation)} $</div>
              </div>
            )}
            {selectedCoin.circulating_supply && (
              <div>
                <p>Circulating Supply:</p>
                <div>{setNumberFormat(+selectedCoin.circulating_supply.toFixed(0))} $</div>
              </div>
            )}
            {selectedCoin.total_supply && (
              <div>
                <p>Total Supply:</p>
                <div>{setNumberFormat(+selectedCoin.total_supply.toFixed(0))} $</div>
              </div>
            )}
