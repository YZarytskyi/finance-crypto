...
export const CheckResult = () => {
  const { currencies, arbitrageResult, pair1, pair2, pair3, isLoadingPairs } = useAppSelector(
    (state) => state.arbitrage,
  )
  ...
  if (currencies.length) {
    ...
    pair2Currencies = allCurrencies.filter((x) => x.includes(filterPair1) && !x.includes('USDT'))
  ...
  const handlePair1 = (e: React.SyntheticEvent, newValue: string | null): void => {
    newValue && dispatch(setPair1(newValue))
  }
  const handlePair2 = (e: React.SyntheticEvent, newValue: string | null): void => {
    newValue && dispatch(setPair2(newValue))
  }
  const handlePair3 = (e: React.SyntheticEvent, newValue: string | null): void => {
        ...
        <ul className={style.checkResultList}>
          <li>
            <p className={style.pairPrice}>
              {arbitrageResult.price1 ? removeFloatNull(arbitrageResult.price1) : 'Price'}
            </p>
            <Autocomplete
              id='pair-1'
          ...
          <li>
            <p className={style.pairPrice}>
              {arbitrageResult.price2 ? removeFloatNull(arbitrageResult.price2) : 'Price'}
            </p>
            <Autocomplete
              id='pair-2'
              disabled={!currencies.length}
          ...
          <li>
            <p className={style.pairPrice}>
              {arbitrageResult.price3 ? removeFloatNull(arbitrageResult.price3) : 'Price'}
            </p>
            <Autocomplete
              id='pair-3'
              disabled={!currencies.length}
