...
import { CoinByQuery, CoinSearchForm, ExchangeSearchForm } from '../types/Types'
...
export const cryptoApi = {
  ...
  async getSearchDataByQuery(query = '') {
    try {
      const { data } = await axios.get(`https://api.coingecko.com/api/v3/search?query=${query}`)
  ...
  async getSelectedCoinMarketChart(coinId: string | undefined, days: number | 'max') {
