...
const schema = yup.object({
  ...
  isAgree: yup.boolean().isTrue('⚠ You have to agree with Terms of Use and Privacy Policy'),
})
...
export const SignUp: React.FC<SignUpProps> = ({ setModalAuthShow }) => {
  ...
  const signUp = async (email: string, password: string) => {
    try {
      ...
      Notify.success('Hi, you signed up successfully')
    } catch (error: any) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className={style.loginForm}>
      ...
      <input
        ...
        className={style.formInput}
      />
      <p className={style.formErrorMessage}>{errors.password?.message || ' '}</p>
      ...
      <SendButton type='submit' variant='outlined' disabled={loading} className={style.formButton}>
        Create an account
      </SendButton>
    </form>
  )
