...
export const fetchMarketsHome = createAsyncThunk('marketsHome/fetch', async () => {
  return (await cryptoApi.getMarketsHome()) as Array<Markets>
})
export const fetchMarkets = createAsyncThunk<Array<Markets>, number>('markets/fetch', async (page = 1) => {
  return (await cryptoApi.getMarkets(page))
})

export const fetchCoinsByQuery = createAsyncThunk('coinsByQuery/fetch', async (query: string) => {
  return (await cryptoApi.getCoinsByQuery(query)) as Array<Markets> | undefined
})

export const fetchCoinsById = createAsyncThunk('coinsById/fetch', async (id: string) => {
...
export const fetchGlobalData = createAsyncThunk('globalData/fetch', async () => {
...
export const { setMarketsTime, setDefaultMarketsTime, removeSelectedCoin, removeMarketChart } =
  cryptoSlice.actions
